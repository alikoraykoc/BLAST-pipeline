...
class AccessionReferenceDownloader:
    ...
    def download_accession_sequences(self, accessions, gene_name, batch_size=20):
        ...
        # CLEAN gene_name - remove any color codes or special characters
        clean_gene_name = gene_name.strip()
        # Remove ANSI color codes if present
        import re
        clean_gene_name = re.sub(r'\x1b\[[0-9;]*[mGK]', '', clean_gene_name)
        # Remove non-alphanumeric characters except underscore and hyphen
        clean_gene_name = re.sub(r'[^\w\-]', '_', clean_gene_name)
        # Remove multiple underscores
        clean_gene_name = re.sub(r'_+', '_', clean_gene_name)
        # Remove leading/trailing underscores
        clean_gene_name = clean_gene_name.strip('_')

        self.log(f"Using cleaned gene name: {clean_gene_name}")

        output_file = os.path.join(self.output_dir, f"{clean_gene_name}_accession_references.fasta")
            ...
            for i in range(0, len(accessions), batch_size):
                ...
                try:
                    ...
                    for record in records:
                        ...
                        # Create enhanced header with species name - use clean gene name
                        record.id = f"{species_name}_{matched_acc}_{clean_gene_name}"
                        record.description = f"{clean_gene_name} from {species_name.replace('_', ' ')} ({matched_acc})"
                    ...
                    for acc in batch:
                        try:
                            ...
                            # Create enhanced header with species name - use clean gene name
                            record.id = f"{species_name}_{acc}_{clean_gene_name}"
                            record.description = f"{clean_gene_name} from {species_name.replace('_', ' ')} ({acc})"
